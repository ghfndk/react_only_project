...
function App() {
    ...
    /* mode : 현재 상태의 모드 지정 */
    /* insert, updatem delete, read, detail 등등 */
    ...
    const ClickArrived = (id) => {
        ...
        setSelectedId(Number(id)); // 선택한 상품의 id로 변경 되었습니다.
        setMode('detail'); /* 상세 보기 모드로 변환 */
    }

    const getProductById = () => {
        // selectedId : 개발자가 클릭한 상품의 id
        // 상품 목로(product)에서 filter 함수를 이용하여 추출합니다. 
    ...
    const [categories, setCategoryies] = useState(categoryList);

    /* 사용자가 카테고리 추가 화면에서 내용을 기입하고, [추가] 버튼을 눌렀습니다. */
    const InsertCategory = (formData) => {
        //formData는 신규 추가할 카테고리 입니다.

        // 파라미터 이름은 CreateCategory.js파일 에서 참조해야 합니다.
        ...
        setCategoryies(totalCategory); // 카테고리 정보 갱신
        setMode('read'); //모드 변경
    }

    /* 정령 기능 추가 */
    /* 기본 정렬 방식으로 name 컬럼으로 오름차순으로 정령할꼐요 */
    const [orderInfo, setOrderInfo] = useState({ column: 'name', ordering: 'asc' });

    /* 정렬 정보를 사용하여 정렬을 수정해주느 함수입니다. */
    ...
    const Ordering = (orderInformation) => {
        ...
        const textColums = ['name', 'category']; // 문자열 컬럼 목록

        const isCharacter = textColums.includes(column); // 문자열 컬럼 목록
        ...
        products.sort((a, b) => {
            ...
            if (isCharacter) { // 문자열 컬럼 
                ...
                return method === 'dcsc' ? aValue - bValue : bValue - aValue;
            }
        });

    }

    /* 사용자가 ''또는 ''을 선택하면 여기 코드가 실행됩니다. */
    ...
    /* 필드 검색 기능 추가 */
    /* 필드 검새시 내가 선택한 category의 영문 이름이 저장되는 state입니다. */
    ...
    return (
        <Card>
            ...
            <Card.Footer>
                <Bottom message={message} onCilckTOBottom={ModeChanged} />
            </Card.Footer>
        </Card>
    );
}
