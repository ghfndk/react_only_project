...
function App({ contents, onClickToContent, categories, onOrderByClick, orderInfo, onChangeCategory }) {
                ...
                {contents.map((item, index) => {
                    ...
                    return (
                        <tr id={item.id} key={index}>
                            ...
                            {/* match가 의미 있는 데이터라면 "한글"을, 아니면 "원래 값"을 표시함 */}
    ...
    const ClickButtonGroup = (event) => {
        ...
        const target_id = event.target.id; // 이벤트 동작을 일으킨 요소의 uniqe한 id 정보

        // oderColumList : 정렬할 컬럼 정보와 관련 있는 배열
        const orderColumList = ['name', 'price', 'category'];

        // 클릭한 항목의 id가 orderColumList 배열에 포함이 되어 있으면 true를 반환해 줍니다.
    ...
    return (
        <>
            <Table>
                <tbody>
                    <tr>
                        {/* width를 10% 설정한 이유는 Dropdown 항목들이 붙어 있는 것처럼 설청하기 위하여 코딩함 */}
