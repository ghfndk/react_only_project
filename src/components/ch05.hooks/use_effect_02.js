...
function App() {
    ...
    const [another, setAnother] = useState(0); // 카운터의 3배수 더하기 1

    const [image, setImage] = useState(`${imagePath}/americano01.png`);
    const [imageIdx, setImageIdx] = useState(0); // 현재 석택된 이미지의 색인 번호
    ...
    const TodoSomething = () => {
        ...
        setAnother(() => 3 * count + 1); // count의 3배수 더하기 1
        ...
        if(imageIdx ===(imageArray.length -1)){ // 배열 범위를 벗어마녀 다시 0으로 초기화
            setImageIdx(0);
        }
    };

    // 최초 1번 갱신(rendering)이 되고 나서, count가 바뀔 때마다 다시 그리기(rendering)를 해줍니다.
