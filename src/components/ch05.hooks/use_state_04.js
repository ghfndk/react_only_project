...
function App() {
    ...
    const RemoveSelectedRow =(event) =>{
        ...
        setMembers(filteredData); // 상태 업데이트로 삭제 반영
    }

    const memberList = members.map((bean) => {
        // 목록 내역을 <li> 태그 형식으로 만들어서 반환합니다.
        // 더브 클릭시 해당 항목을 목록에서 제거 합니다.
    ...
    const AddMember = (event) => {
        // members 배열에서 id 황목들만 추출합니다.
        // 이 결과 역시 객체로 되어 있는 데, ... 연산자를 사용하여 스칼라 데이터로 만든다음
        // macx() 메소드를 이용하여 최대 값을 추출합니다.
